...
**Питання**: Як забезпечуєте продуктивність при відображенні тисяч рядків? Різниця між client-side та server-side пагінацією?

**Оптимальна відповідь**:
- **Virtual scrolling** (ag-Grid `rowModelType: 'infinite'`): рендерить лише видимі рядки (~20-30), решта підвантажуються при скролі. DOM залишається легким
- **Server-side**: `rowModelType: 'serverSide'` — дані завантажуються порціями з API (`skip/take`), фільтрація/сортування виконується на сервері (SQL). Для 100k+ записів — єдиний варіант
- **Client-side**: завантажити всі дані одразу, фільтрація у браузері. Підходить для <5k записів
- **Коли що**: client-side для довідників (валюти, статуси). Server-side для транзакцій, замовлень, аудит-логів
- **Додатково**: debounce на фільтрах (300ms), кешування завантажених сторінок, `OnPush` change detection для Angular-компонента з гридом

---

### Q13 (5 хв): State management в Angular
**Питання**: Як керуєте станом? У порталі де роль, tenant context та сесія мають бути доступні скрізь — як структуруєте?

**Оптимальна відповідь**:
- Для невеликих/середніх проєктів: **сервіси з `BehaviorSubject`** — простіше ніж NgRx, достатньо для більшості кейсів
- `AuthService` (Singleton): `BehaviorSubject<User>` — роль, tenant, токен. Наповнюється після логіну, читається через `user$` observable
- `TenantContextService`: `BehaviorSubject<TenantInfo>` — поточний банк-клієнт, конфігурація, ліміти
- Для складної бізнес-логіки або коли один стан впливає на багато компонентів: **NgRx** (Redux-патерн) — actions, reducers, effects, selectors. Передбачуваність, DevTools для дебагу
- **Комбінований підхід**: глобальний стан (auth, tenant) через NgRx, локальний стан компонента (форми, фільтри) через сервіси або `@Input`/`@Output`
- **Route Guards**: `canActivate` перевіряє роль через `AuthService`, але це лише UX-захист. Реальна авторизація — на бекенді
