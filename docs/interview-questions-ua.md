...
### Q13 (5 хв): State management в React
**Питання**: Як керуєте глобальним станом в React екосистемі? У порталі де роль, tenant context та сесія мають бути доступні скрізь — як структуруєте? Яка різниця між client state та server state?

**Оптимальна відповідь**:
- Для базових глобальних речей: **React Context API** — ідеально та просто для сесії, теми або локалізації (дані які рідко змінюються).
- `AuthContext` та `TenantContext`: глобальні провайдери. Зберігають поточну роль, конфігурацію клієнта-банку.
- Для складного client-state (складні форми, корзина тощо): **Redux Toolkit (RTK)** або сучасніші легкі альтернативи на зразок **Zustand**. Забезпечують передбачуваність, DevTools для дебагу, уникають prop drilling та context hell.
- **Server State (API дані)**: банківські транзакції або списки замовлень краще не зберігати в Redux. Ефективніше використовувати **React Query (TanStack Query)** або RTK Query — вони беруть на себе автоматичне кешування, ревалідацію та loading/error статуси.
- **Route Guards**: захист сторінок у React Router реалізується через кастомні HOC або враппери (наприклад, `<ProtectedRoute>`), які перевіряють контекст авторизації. Але справжня безпека завжди залишається на бекенді.
